...
def regression(train_set, test_set, train_labels, test_labels, step=0.001, max_epochs=140, batch_size=2):
    ...
    W2, b2 = tf.keras.backend.random_normal_variable(shape=[int(sample_len/2), 1], mean=0.1, scale=1), tf.keras.backend.random_normal_variable(shape=[1], mean=0.1, scale=1)
...
#     'inflation_budget', 'scaled_budget', 'budget_num_cast_ratio', 'budget_runtime_ratio']
    ...
    dropped = final_dropped + tentative_drop0
